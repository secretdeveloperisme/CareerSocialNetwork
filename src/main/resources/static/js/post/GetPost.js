...
import caculate_reading_time from "../utils/caculate_reading_time.js";
...
$(() => {
  ...
  $readTime.text(caculate_reading_time($postContent.text()))
  ...
  function replyComment() {
    ...
    $formReply.on("submit", event => {
      ...
      let $postId = $("<input type='hidden' name='postId'>").val($(this).data('post-id'));
      $formReply.append($commentContent).append($parentId).append($postId);
      event.preventDefault()
      $.ajax({
        type: "POST",
        url: "/api/comment/",
              ...
              <div class="d-flex comment-wrapper--deep-0">
                ...
                <div class="comment-detail p-3 border border-bold w-100 mb-2">
                  ...
                  <div class="comment-action">
                    ...
                    <button class="btn comment-reply-btn" id="reply-comment-id-${response.data.comment.commentId}" data-post-id="${response.data.comment.postId}" data-comment-id="${response.data.comment.commentId}"><i class="far fa-comment"></i> Reply</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  $btnFollow.on("click", () => {
    ...
    $.ajax({
      type: "POST",
      url: "/api/follow-user",
      data:JSON.stringify({followed_id:userId}),
      contentType: "application/json",
      success: function (response) {
